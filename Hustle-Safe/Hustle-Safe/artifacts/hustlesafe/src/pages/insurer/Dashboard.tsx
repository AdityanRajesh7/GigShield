...
import { useListZones, useTriggerDisruption, useResolveDisruption, useListClaims } from "@workspace/api-client-react";
...
export function InsurerDashboard() {
  ...
  const { data: claimsData } = useListClaims({ limit: 8 }, { query: { refetchInterval: 3000 } });
  ...
  const handleTrigger = async () => {
    ...
    try {
      ...
      toast.success("Disruption Triggered", {
        description: `Zone signals updated. Payouts generating.`
      });
      ...
      <div className="space-y-8 pb-12">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-muted-foreground mt-1">System-wide monitoring and manual overrides.</p>
          </div>
        ...
        <div className="grid grid-cols-5 gap-4">
          {[
            ...
            <div key={i} className="bg-card border border-border rounded-2xl p-4 shadow-sm">
              <div className="text-xs font-bold text-muted-foreground uppercase tracking-wider mb-2">{kpi.label}</div>
              <div className="text-2xl font-display font-bold text-foreground mb-1">{kpi.value}</div>
              <div className="text-[10px] font-bold text-primary uppercase">{kpi.sub}</div>
            </div>
          ))}
        </div>
          ...
          <div className="space-y-6">
            <div className="bg-foreground text-background rounded-3xl p-6 shadow-xl relative overflow-hidden">
              ...
              <div className="space-y-6 relative z-10">
                <div>
                  <label className="text-xs font-bold text-white/60 uppercase tracking-wider mb-2 block">Target Zone</label>
                  <select 
                ...
                <div>
                  <label className="text-xs font-bold text-white/60 uppercase tracking-wider mb-3 block">Event Type</label>
                ...
                <div>
                  <div className="flex justify-between items-center mb-3">
                    <label className="text-xs font-bold text-white/60 uppercase tracking-wider block">GDS Target</label>
                    <span className="font-display font-bold text-xl">{gdsTarget}</span>
                  </div>
                  <Slider 
                    value={[gdsTarget]} 
                ...
                <div>
                  <div className="flex justify-between items-center mb-3">
                    <label className="text-xs font-bold text-white/60 uppercase tracking-wider block">Duration (Mins)</label>
                ...
                >
                  {triggerMutation.isPending ? "Triggering..." : "TRIGGER DISRUPTION"}
                </Button>
              </div>
            </div>
            
            <div className="bg-card rounded-3xl p-6 border border-border shadow-sm">
              <h3 className="font-bold text-sm uppercase tracking-wider text-muted-foreground mb-4">Reserve Health</h3>
              <div className="mb-2 flex justify-between items-end">
                <span className="text-2xl font-display font-bold">₹8.4L</span>
                <span className="text-sm font-medium text-muted-foreground">Target: ₹12L</span>
              </div>
